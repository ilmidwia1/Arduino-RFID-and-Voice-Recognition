...
#define rst_mfrc522_1 9
#define ss_mfrc522_1 10
MFRC522 mfrc522_1(ss_mfrc522_1, rst_mfrc522_1);
String kode_rfid_mfrc522_1 = "";

bool bacaRFID_mfrc522_1() {
  if (mfrc522_1.PICC_IsNewCardPresent() && mfrc522_1.PICC_ReadCardSerial()) {
    kode_rfid_mfrc522_1 = "";
    for (byte i = 0; i < mfrc522_1.uid.size; i++) {
      kode_rfid_mfrc522_1 += String(mfrc522_1.uid.uidByte[i] < 0x10 ? "0" : "");
      kode_rfid_mfrc522_1 += String(mfrc522_1.uid.uidByte[i], HEX);
    }
...
void setup(void)
{
  ...
  if (myVR.clear() == 0) {
    ...
    lcd.setCursor(0, 0);
    lcd.print("Gagal,Tolong Reset ..");
    Serial.println("Not find VoiceRecognitionModule.");
    Serial.println("Please check connection and restart Arduino.");
    while (1);
  }

  if (myVR.load((uint8_t)bukaPintu) >= 0) {
...
void loop(void)
{
  ...
  if (bacaRFID_mfrc522_1()) {
  Serial.print("Kartu Terdeteksi: ");
  Serial.println(kode_rfid_mfrc522_1);

  if (kode_rfid_mfrc522_1 == "2776e6a7")
  {
bipbip();
        lcd.setCursor(0, 0);
        lcd.print("RFID Sesuai..");
        lcd.setCursor(0, 1);
...
void printTrain(uint8_t *buf, uint8_t len)
{
  if (len == 0) {
    ...
    lcd.setCursor(0, 0);
    lcd.print("Train Finish.");
    return;
  }
  else {
    Serial.print(F("Train success: "));
    lcd.setCursor(0, 0);
    lcd.print("Train success:");
    biiip();
    lcd.setCursor(0, 1);
    lcd.print("Silahkan Reset ..");
    Serial.println(buf[0], DEC);
  }
  for (int i = 0; i < len - 1; i += 2) {
    Serial.print(F("Record "));
